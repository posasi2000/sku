...
//====다 체크하고 submit
	function check(){
		...
		var pp= document.getElementById("pwdlabel");
		...
		if (pwd==null||pwd==""){
			alert('비밀번호를 입력하세요.');
			//pp.innerHTML = "비밀번호를 입력하세요";
			iform.pwd.focus();
			return;
		}
		if (pwd2==null||pwd2==""){
			//alert('비밀번호를 재입력하세요.');
			pp.innerHTML = "확인비밀번호를 재입력하세요";
			iform.pwd2.focus();
			return;
		}
		
		if (ff==null||ff==""){ //파일선택널체크
			alert('파일선택을 하셔야 합니다.');
			iform.file.focus();
			return;
		}
		...
		//flagS = false;//사번중복체크여부 flagP = false;//비번확인체크여부 
		if(flagS==true&&flagP==true){
	...
	function checkSb(){//사번 입력체크하고 중복확인 팝업창 열어주기
		...
		label.innerHTML = "";
		flagS=true;
		open('openSb.jsp','oepnSb','width=600, height=150, left=300, top=300');
	}//checkSb end
	
	//openSbSave.jsp문서에서 사용
    function flagS_t(){ flagS=true; }
	
	
    //====비밀번호 확인
	function checkPwd(){//비밀번호 확인 체크
		var str = document.getElementById("pwdlabel");
		var a = iform.pwd.value;
		var b = iform.pwd2.value;
		//alert("pwd=" +a+" pwd2=" +b);
...
//====각각 데이터 입력 자리수제한(db오류 안나게 바이트로 처리)
	function checklen(obj, maxByte)	{	//db데이터 사이즈에 맞는 길이 체크
		...
		if (totalByte > maxByte) {
			alert(maxByte+" Byte를 초과 입력 불가합니다");
